...
public class HttpComponentClientTemplate<Object> implements HttpClientTemplate {
	...
	/**
	 * HttpClientException 발생 시 throw하지 않고 디폴트값(리턴타입이 String인 경우는 공백, 다른 타입의 경우는
	 * null)으로 리턴하는 메소드 (client 코드에서 특별히 처리해줄 것이 없을 때 사용한다)
	 * 
	 * @param requestParameters
	 * @return
	 * @throws IOException
	...
	protected CloseableHttpClient createHttpClient() {
		...
		// TODO : PoolingHttpClientConnectionManager 도 확인 필요.
	...
	private HttpUriRequest createHttpMethod(String methodType, HttpClientParameters parameters) throws HttpClientException {
		...
		if (GET_METHOD.equalsIgnoreCase(methodType)) {
			...
			// addParameters로 추가되면 body를 먼저 지우고 파라미터가 설정된다. 하단의 entity 설정시도
			// clear 후 설정된다.
			/*
		 ...
		 * EntityEnclosingMethod) { // entity로 body에 설정되면 addParameters로 설정된
		 * body는 클리어 된다. processEntityEnclosingMethod(httpParameters,
		 * httpMethod); }
		 */
